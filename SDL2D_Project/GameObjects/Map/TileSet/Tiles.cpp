...
//{
//}
TileSetManagerr::TileSetManagerr()
{
...
TileSetManagerr* TileSetManagerr::GetInstance()
{
	if (instance != nullptr)
	{
		return instance;
	...
	{
		instance = new TileSetManagerr();
		return instance;
	}
}

TileSetManagerr::TileSetManagerr(const char* dir_, std::string name_, int ID_)
{
...
void TileSetManagerr::CreateSet(int sizeOfCut_, int srcX_, int srcY_)
{
//	imageSetHolder = TextureManager::CreateMapSprite(tileSetImage, width, height, sizeOfCut_, srcX_, srcY_);
}


void TileSetManagerr::SetDisplayRects(SDL_Rect tempSrcR_, SDL_Rect tempDesR_)
{
	srcRect = tempSrcR_;
	desRect = tempDesR_;
}

void TileSetManagerr::SetBaseImage(SDL_Texture* tempIMG_)
{
	baseImage = tempIMG_;
}

SDL_Texture* TileSetManagerr::getBaseImage()
{
	return baseImage;
}
...
TILE::TILE() : collider(nullptr) , sprite(nullptr) , transform(nullptr)
{
}
